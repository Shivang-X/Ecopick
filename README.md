...
```bash
docker build -t docker.io/shivangx ecopick-backend:$CI_COMMIT_SHORT_SHA -f ./Dockerfile .
```

- Push the docker container

```bash
docker push docker.io/shivangx/ecopick-backend:$CI_COMMIT_SHORT_SHA
```
...
```bash
docker run -d -p 8080:8080 --name ecopick-backend docker.io/shivangx/ecopick-backend:$CI_COMMIT_SHORT_SHA
```
